...
public class SubtaskResultToFileSinker extends RichSinkFunction<QueryPair> {
    public String sinkDir;

    public SubtaskResultToFileSinker(String sinkDir) {
        ...
        for (File sinkFile : sinkDirFile.listFiles()) {
            ...
            if(mergedList.size() != 0) {
                ...
                for(long time:mergedList){
                    ...
                    map.put((int) (time%1000),map.getOrDefault((int)time%1000,0L)+time);
                }
            }
        }
        long avgTime = 0;
